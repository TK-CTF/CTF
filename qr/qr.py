...
def hammingDistance(s1, s2):
    if len(s1) != len(s2):
        sys.stderr.write('error: 文字列の長さが違います\n')
        sys.exit(1)
    r = 0
...
def mask(pat, x, y):
    ...
    sys.stderr.write('error: 不正なパターンです\n')
    sys.exit(1)
...
def detGF256(A):
    if len(A) != len(A[0]):
        sys.stderr.write('error: 正方行列ではありません\n')
        sys.exit(1)
    size = len(A)
    if size == 1:
        return A[0][0]
...
def solveSE(A):
    ...
    for i in range(size):
        ...
        if A[-(i + 1)][-2 - i] == 0b00000000:  # 解が一意に定まらないため、誤り訂正失敗
            sys.stderr.write('error: 誤り訂正に失敗しました。--no-correctionオプションの仕様を検討してください\n')
            sys.exit(1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-e', '--error-correction', help='エラー訂正レベル(1:L 0:M 3:Q 2:H)')
    parser.add_argument('-m', '--mask', help='マスクパターン(0〜7)')
    parser.add_argument('-n', '--no-correction', action='store_true', help='データブロックの誤り訂正をしない')
    parser.add_argument('-v', '--verbose', action='store_true', help='詳細な情報を表示')
    parser.add_argument('FILE', nargs='?', default='-', help='入力ファイル(デフォルトは標準入力)')
    ...
    for i in range(width):
        if len(lines[i]) != width:
            sys.stderr.write('error: 入力が正方形ではありません\n')
            sys.exit(1)
    # 型番(version)の計算
    if (width - 21) % 4 != 0 or width < 21 or 177 < width:
        sys.stderr.write('error: 辺の長さがおかしいです\n')
        sys.exit(1)
    version = (width - 21) // 4 + 1
    if args.verbose:
        print('型番:\t{0} ({1} x {1})'.format(version, width))
    # 2次元配列data[x][y]の作成
    data = []
    for i in range(width):
        data.append([])
    for line in lines:
        for i in range(len(line)):
            if line[i] in 'XxOo#1':  # 暗モジュール
                ...
                sys.stderr.write('error: 不正な文字が含まれています\n')
                sys.exit(1)
    # 読み込んだデータの表示
    if args.verbose:
        print('読み込んだデータ:')
    ...
    for i in range(8):
        if data[i][7] == 1 or data[7][i] == 1 or data[width - 8 + i][7] == 1 or data[width - 8][i] == 1 or data[i][
            width - 8] == 1 or data[7][width - 8 + i] == 1:
    ...
    for x in array:
        for y in array:
            if not ((x < 9 and y < 9) or (width - 10 < x and y < 9) or (
                    x < 9 and width - 10 < y)):  # 位置検出パターンと重なる場合は除外する
    ...
    if 7 <= version:
        ...
        if args.verbose:
            print('型番情報(右上):\t\t{0}'.format(right_top))
        # 2つの型番情報の合成
        composed = ''
        diff_format = False
        for i in range(18):
        ...
        if args.verbose:
            if diff_format:
                ...
                stdoutColor('[ OK ]', 'green')
            print(' 2つの型番情報の整合性')
            print('型番情報(合成):\t\t{0}'.format(composed))
        # 誤り訂正
        ...
        if index == -1:  # 誤り訂正失敗
            ...
            if args.verbose:
                print('型番情報(誤り訂正後):\t{0}'.format(version_information))
    ...
    if args.verbose:
        if diff_format:
            ...
            stdoutColor('[ OK ]', 'green')
        print(' 縦横の形式情報の整合性')
        print('形式情報(合成):\t\t{0}'.format(composed))
    composed_unmask = ''.join(
        ['0' if x == y else '1' if x in '01' and y in '01' else '?' for (x, y) in zip(composed, format_mask)])
    if args.verbose:
        print('形式情報(マスク解除):\t{0}'.format(composed_unmask))
    # 誤り訂正
    ...
    if index == -1:  # 誤り訂正失敗
        ...
        if '?' in composed_unmask[:5] and not (
                args.error_correction != None and args.mask != None):  # データ部が全て不明で、引数指定もなし
            sys.stderr.write('error: 形式情報のデータ部が不明、かつ誤り訂正に失敗しました\n')
            sys.exit(1)
        ...
        if args.verbose:
            print('形式情報(誤り訂正後):\t{0}'.format(format_information))
    ...
    if args.verbose:
        ...
        print('マスクパターン:\t{0}\t[ {1} ]'.format(mask_pattern, m))
    ...
    if args.verbose:
        ...
        print('データブロック: {0}'.format(repr(blocks)))
    # RSブロックに分割する
    RS_blocks = []
    ...
    for i in range(block_num):
        ...
        RS_blocks.append(t)
    if args.verbose:
        print('RSブロック数:\t{0}'.format(len(RS_blocks)))
        print('RSブロック分割後:')
    ...
    if args.verbose:
        print('各RSブロック中の不明データコード数: {0}'.format(repr(unknown_code_nums)))
    # 不明モジュールを0で埋める
    for i in range(block_num):
    ...
    for i in range(block_num):  # 各RSブロックについて個別に誤り訂正する
        ...
        sigmas.reverse()
        if args.verbose:
            print('誤り位置多項式の係数: {0}'.format(repr(sigmas)))
        # 誤り位置の計算
        indexes = []
        for j in range(len(RS_blocks[i])):
            ...
            if s == 0b00000000:
                indexes.append(j)
        if args.verbose:
            print('誤り位置: {0}'.format(repr(indexes)))
        # 誤りの大きさを求める
        A = []
        ...
        errors = solveSE(A)
        if args.verbose:
            print('誤りの大きさ: {0}'.format(repr(['{0:08b}'.format(x) for x in errors])))
        # 誤りを訂正する
        for j in range(len(indexes)):
            RS_blocks[i][indexes[j]] ^= errors[j]
        if args.verbose:
            print('誤り訂正後のRSブロック: {0}'.format(repr(['{0:08b}'.format(x) for x in RS_blocks[i]])))
    ...
    if args.verbose:
        print('最終的なデータビット列: {0}'.format(data_bits))
    # データを読み終えるまで繰り返し
    data = []
    while len(data_bits) != 0:
        ...
        if mode == 0b0001:
            ...
            sys.stderr.write('error: 未対応のモード指示子です\n')
            sys.exit(1)
        if args.verbose:
        ...
        if mode == 0b0001:
            ...
            if args.verbose:
                print('デコードされたデータ: {0}'.format(repr(list(map(hex, data)))))
        # 英数字のとき
        if mode == 0b0010:
            # 文字数指示子を取得
            if version <= 9:
